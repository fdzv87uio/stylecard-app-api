...
export async function getAllCachedProducts(gender: string) {
    ...
    const productList = await Product.aggregate([{ $match: { $or: [{ brand_name: { $eq: "Gant" } }, { brand_name: { $eq: "Dresslily" } }, { brand_name: { $eq: "Joules" } }, { brand_name: { $eq: "Ted Baker" } }, { brand_name: { $eq: "Hello Molly" } }, { brand_name: { $eq: "Sweaty Betty" } }, { gender: { $eq: gender } }] } }, { $sample: { size: 1000 } }]);
