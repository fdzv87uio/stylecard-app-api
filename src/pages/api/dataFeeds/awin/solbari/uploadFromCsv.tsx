...
apiRoute.post(async (req, res) => {
    try {
        ...
        docs.forEach(async function (item: any, key: number) {
            ...
            const colorArray = item.mpn.split("-");
            console.log("brand: " + item.brand_name);
            const currentSizeGuide = SizeGuidesByBrand.filter((x) => x.brand_name === "SOLBARI Sun Protection")[0];
            ...
            const currentSize = currentSizeArray.filter((x) => x.size_name === colorArray[1])[0];
